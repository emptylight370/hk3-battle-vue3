...
type WorkerResponse = { type: 'progress'; done: number; total: number } | { type: 'done'; result: BatchResult };

/** 在 Worker 线程批量对局；onProgress 收到每 500 场的进度上报 */
export function runBatch(req: BatchRequest, onProgress?: (done: number, total: number) => void): Promise<BatchResult> {
